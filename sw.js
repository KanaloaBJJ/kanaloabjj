const CACHE_NAME='kanaloa-bjj-v13-4';
...
self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(k=>Promise.all(k.filter(x=>x!==CACHE_NAME).map(x=>c.delete(x)))).then(()=>self.clients.claim()))});
